...
const httpTrigger: AzureFunction = async function (context: Context, req: HttpRequest): Promise<void> {
  ...
  try {
    ...
    const servicePrincipal = await client.api(`/servicePrincipals/${resourceId}`).get();

    const servicePrincipalAppRoles: AppRole[] = servicePrincipal.appRoles || []
    const servicePrincipalUserAppRoles = servicePrincipalAppRoles.filter((servicePrincipalAppRole: AppRole) => servicePrincipalAppRole.allowedMemberTypes.includes('User'))

    // Remove everything not relevant to the frontend
    const returnValue  = servicePrincipalUserAppRoles.map(servicePrincipalUserAppRole => ({ 
